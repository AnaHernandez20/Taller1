...
public class CalculadoraEstadisticas
{
	...
	public String darPaisAtleta(String nombreBuscar) {
		
		Atleta atletaEncontrado = null;
		String pais;
		
		for (Atleta cadaAtleta : this.atletas) {
			
			if (cadaAtleta.darNombre().equals(nombreBuscar)) {
				
				atletaEncontrado = cadaAtleta;
			}
		}
		
		if (!(atletaEncontrado == null)) {
			pais = "El pais del atleta "+nombreBuscar+" es "+atletaEncontrado.darPais().darNombre();
		}
		else {
			pais = "No existe un atleta con el nombre "+nombreBuscar;
		}
